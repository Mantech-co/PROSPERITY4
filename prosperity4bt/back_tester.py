...
class BackTester:
    ...
    def run(self):
        algorithm_label = self.options.algorithm_path if self.options.algorithm_path else "Trader class"
        print(f"running algorithm '{algorithm_label}'...")

        trader_module, trader_class = self.__load_algorithm()
        ...
        for round in round_days_options:
            for day in round.days:
                print(f"Backtesting {algorithm_label} for round: {round.round} day: {day}")
                result = self.__run_test(trader_module, trader_class, data_reader, round.round, day)
    ...
    def __load_algorithm(self) -> tuple[Optional[Any], Any]:
        if self.options.trader_class is not None:
            return None, self.options.trader_class

        if self.options.algorithm_path is None:
            print("Error: neither algorithm_path nor trader_class provided")
            sys.exit(1)
        ...
        return trader_module, trader_module.Trader
    ...
    def __run_test(self, trader_module: Optional[Any], trader_class: Any, data_reader: BackDataReader, round: int, day: int) -> BacktestResult:
        if trader_module is not None:
            reload(trader_module)
            trader_instance = trader_module.Trader()
        else:
