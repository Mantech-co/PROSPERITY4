...
CONTRACTS = [
    {"name": "AC",        "expiry": "N/A",     "kind": "underlying", "K": 50,  "bid": 49.975, "ask": 50.025, "max": 200, "aon": None, "expiry_day": 21, "choose_day": None, "barrier": None},
    {"name": "AC_50_P",   "expiry": "T+21",    "kind": "put",        "K": 50,  "bid": 12,     "ask": 12.05,  "max": 50,  "aon": None, "expiry_day": 21, "choose_day": None, "barrier": None},
    {"name": "AC_50_C",   "expiry": "T+21",    "kind": "call",       "K": 50,  "bid": 12,     "ask": 12.05,  "max": 50,  "aon": None, "expiry_day": 21, "choose_day": None, "barrier": None},
    {"name": "AC_35_P",   "expiry": "T+21",    "kind": "put",        "K": 35,  "bid": 4.33,   "ask": 4.35,   "max": 50,  "aon": None, "expiry_day": 21, "choose_day": None, "barrier": None},
    {"name": "AC_40_P",   "expiry": "T+21",    "kind": "put",        "K": 40,  "bid": 6.5,    "ask": 6.55,   "max": 50,  "aon": None, "expiry_day": 21, "choose_day": None, "barrier": None},
    {"name": "AC_45_P",   "expiry": "T+21",    "kind": "put",        "K": 45,  "bid": 9.05,   "ask": 9.1,    "max": 50,  "aon": None, "expiry_day": 21, "choose_day": None, "barrier": None},
    {"name": "AC_60_C",   "expiry": "T+21",    "kind": "call",       "K": 60,  "bid": 8.8,    "ask": 8.85,   "max": 50,  "aon": None, "expiry_day": 21, "choose_day": None, "barrier": None},
    {"name": "AC_50_P_2", "expiry": "T+14",    "kind": "put",        "K": 50,  "bid": 9.7,    "ask": 9.75,   "max": 50,  "aon": None, "expiry_day": 14, "choose_day": None, "barrier": None},
    {"name": "AC_50_C_2", "expiry": "T+14",    "kind": "call",       "K": 50,  "bid": 9.7,    "ask": 9.75,   "max": 50,  "aon": None, "expiry_day": 14, "choose_day": None, "barrier": None},
    {"name": "AC_50_CO",  "expiry": "T+14/21", "kind": "chooser",    "K": 50,  "bid": 22.2,   "ask": 22.3,   "max": 50,  "aon": None, "expiry_day": 21, "choose_day": 14,   "barrier": None},
    {"name": "AC_40_BP",  "expiry": "T+21",    "kind": "aon_put",    "K": 40,  "bid": 5.0,    "ask": 5.1,    "max": 50,  "aon": 10,   "expiry_day": 21, "choose_day": None, "barrier": None},
    {"name": "AC_45_KO",  "expiry": "T+21",    "kind": "ko_put",     "K": 45,  "bid": 0.15,   "ask": 0.175,  "max": 500, "aon": None, "expiry_day": 21, "choose_day": None, "barrier": 35},
]
...
def _contract_pnl(c, S, side, vol):
    """
    Compute per-path PnL for one contract.
    S: full path array (n_paths, total_steps+1), steps per day = s implied by expiry_day.
    """
    s_per_step = (S.shape[1] - 1) // c["expiry_day"] if c["expiry_day"] > 0 else 1
    expiry_step = c["expiry_day"] * s_per_step
    S_T = S[:, expiry_step]
    ...
    if kind == "call":
        ...
        choose_step = c["choose_day"] * s_per_step
...
def mc_portfolio(
    ...
    s         : steps per day
    N_max     : simulation horizon in days (should be >= longest expiry_day)
    ...
    for name, (side, vol) in positions.items():
        c = CONTRACT_MAP[name]
        vol = min(vol, c["max"])
        pnl = _contract_pnl(c, S, side, vol)
        portfolio_pnl += pnl
...
if __name__ == "__main__":
    ...
    print(f"S0=50  sigma=0.1582  s=4  N_max=21  n_paths={N_PATHS:,}\n")
