...
class LinearComboPanel(QWidget):
    """Side panel shown when '[Linear Combo]' is selected as product.
    Displays one coefficient QLineEdit per asset and a Plot button."""
    plot_requested = pyqtSignal(dict)   # emits {asset: coeff, ...}
    ...
    def __init__(self, parent=None):
        ...
        self._lbl_mean1 = _stat_lbl()   # mean + 1σ
        self._lbl_mean2 = _stat_lbl()   # mean − 1σ
    ...
    def update_stats(self, mean: float, std: float):
        """Update the stats labels with computed values."""
    ...
    def set_assets(self, assets: list[str]):
        """Rebuild rows for the given asset list, preserving existing values."""
...
class LogVisualizer(QMainWindow):
    def __init__(self, log_path=None):
        ...
        self._combo_curve = None          # PlotDataItem for linear combo mid
        self._combo_mean_line = None      # InfiniteLine for combo mean
    ...
    def _build_ui(self):
        ...
        # Horizontal area: optional LinearComboPanel + graph
    ...
    def _process_selection(self):
        ...
        if self._combo_mean_line:
            self._combo_mean_line.setVisible(False)
        self._combo_panel.reset_stats()
        self.p_m.setTitle("")   # clear any combo formula title
    ...
    def _plot_linear_combo(self, coeffs: dict):
        """Compute and plot weighted sum of mid prices for the given coefficients."""
        if not self.data:
            return
        day = self.cb_day.currentText()
        ...
        # Build (time, mid) arrays per asset, apply day filter
        ...
        # Union of all timestamps, then interpolate each series onto it
        ...
        # Ensure combo curve exists and is plotted
        ...
        # Mean line — create once, then just reposition
        ...
        # Build a label showing the formula
