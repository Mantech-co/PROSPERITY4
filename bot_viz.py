...
class LinearComboPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        title = QLabel("Linear Combination")
        title.setStyleSheet(f"color: {ACCENT_CYAN}; font-weight: bold; font-size: 9pt; border: none;")
        outer.addWidget(title)

        hint = QLabel("Combo = Σ coeff × mid_price")
        hint.setStyleSheet(f"color: {DIM}; font-size: 7.5pt; border: none;")
        ...
        self._coeff_inputs: dict[str, QLineEdit] = {}

        btn_plot = QPushButton("▶ Plot Combo")
        btn_plot.setStyleSheet(
            f"QPushButton {{ background: {ACCENT_CYAN}; color: #000; font-weight: bold; border-radius: 3px; padding: 5px; }}"
            f"QPushButton:hover {{ background: #00eeff; }}"
        )
        btn_plot.clicked.connect(self._emit_plot)
        outer.addWidget(btn_plot)

        btn_clear = QPushButton("✕ Clear Coefficients")
        btn_clear.clicked.connect(self._clear_coeffs)
        outer.addWidget(btn_clear)

        sep = QFrame(); sep.setFrameShape(QFrame.Shape.HLine)
        sep.setStyleSheet(f"color: {BORDER}; border: none; border-top: 1px solid {BORDER};")
        outer.addWidget(sep)

        stats_title = QLabel("Spread Statistics")
        stats_title.setStyleSheet(f"color: {ACCENT_CYAN}; font-weight: bold; font-size: 8pt; border: none;")
        outer.addWidget(stats_title)

        def _stat_lbl():
            lbl = QLabel("—")
            lbl.setStyleSheet(f"color: {TEXT}; font-size: 8.5pt; border: none; font-family: 'JetBrains Mono', monospace;")
            return lbl
        
        self._lbl_mean  = _stat_lbl()
        self._lbl_std   = _stat_lbl()
        self._lbl_mean1 = _stat_lbl()
        self._lbl_mean2 = _stat_lbl()

        for key_text, widget in [
            ("μ (mean) :", self._lbl_mean),
            ("σ (std)  :", self._lbl_std),
            ("μ + σ    :", self._lbl_mean1),
            ("μ − σ    :", self._lbl_mean2),
        ]:
            ...
            lk = QLabel(key_text)
            lk.setStyleSheet(f"color: {DIM}; font-size: 7.5pt; border: none; font-family: 'JetBrains Mono', monospace;")
            hl.addWidget(lk); hl.addWidget(widget)
            outer.addWidget(row_w)

    def update_stats(self, mean: float, std: float):
        self._lbl_mean.setText(f"{mean:.4f}")
        self._lbl_std.setText(f"{std:.4f}")
        self._lbl_mean1.setText(f"{mean + std:.4f}")
        self._lbl_mean2.setText(f"{mean - std:.4f}")
        self._lbl_mean.setStyleSheet(f"color: {ACCENT_GOLD}; font-size: 8.5pt; border: none; font-weight: bold; font-family: 'JetBrains Mono', monospace;")

    def reset_stats(self):
        for lbl in [self._lbl_mean, self._lbl_std, self._lbl_mean1, self._lbl_mean2]:
            lbl.setText("—")
            lbl.setStyleSheet(f"color: {TEXT}; font-size: 8.5pt; border: none; font-family: 'JetBrains Mono', monospace;")

    def set_assets(self, assets: list[str]):
        old_vals = {a: w.text() for a, w in self._coeff_inputs.items()}
        while self._rows_layout.count() > 1:
            item = self._rows_layout.takeAt(0)
            if item.widget():
                item.widget().deleteLater()
        self._coeff_inputs.clear()
        ...
        for asset in sorted(assets):
            ...
            hl.setSpacing(6)
            lbl = QLabel(asset)
            lbl.setStyleSheet(f"color: {TEXT}; font-size: 8pt; border: none;")
            lbl.setFixedWidth(100)
            lbl.setToolTip(asset)
            inp = QLineEdit(old_vals.get(asset, "0"))
            inp.setFixedWidth(60)
            inp.setPlaceholderText("coeff")
            inp.setAlignment(Qt.AlignmentFlag.AlignRight)
            hl.addWidget(lbl)
            hl.addWidget(inp)
            self._coeff_inputs[asset] = inp
            self._rows_layout.insertWidget(self._rows_layout.count() - 1, row)

    def get_coefficients(self) -> dict:
        result = {}
        for asset, inp in self._coeff_inputs.items():
            try:
                val = float(inp.text())
            except ValueError:
                val = 0.0
    ...
    def _emit_plot(self):
        ...
        if not coeffs:
            return
        self.plot_requested.emit(coeffs)

    def _clear_coeffs(self):
        for inp in self._coeff_inputs.values():
            inp.setText("0")
...
class LogVisualizer(QMainWindow):
    def __init__(self, log_path=None):
        ...
        self._combo_curve = None
        self._combo_mean_line = None
        self.bot_scatters = []
        self.bot_colors = {}
    ...
    def _build_ui(self):
        ...
        market_body = QWidget(); market_body_layout = QHBoxLayout(market_body)
        market_body_layout.setContentsMargins(0, 0, 0, 0); market_body_layout.setSpacing(0)
        self._combo_panel = LinearComboPanel(); self._combo_panel.setVisible(False)
        self._combo_panel.plot_requested.connect(self._plot_linear_combo)
        market_body_layout.addWidget(self._combo_panel)
        self.gw_m = pg.GraphicsLayoutWidget(); self.gw_m.setBackground(BG)
        market_body_layout.addWidget(self.gw_m, stretch=1)
        market_layout.addWidget(market_body)
        ...
        for item in [self.sc_buy, self.sc_sell]: self.p_m.addItem(item)
        ...
        self.leg_m.addItem(self.curve_mid, "Mid Price"); self.leg_m.addItem(self.sc_buy, "My Buy"); self.leg_m.addItem(self.sc_sell, "My Sell")
        ...
        self.cb_pnl_type.currentTextChanged.connect(self._handle_product_or_day_change)
    ...
    def _open_data_setup(self):
        ...
        if dlg.exec():
            ...
            self.custom_curves = {}
            self._handle_product_or_day_change()
    ...
    def _save_window_state(self):
        ...
        if self.data:
            ...
            for sc in getattr(self, 'bot_scatters', []):
                legend_vis[f"bot:{sc.name()}"] = sc.isVisible()
            cfg['view_state'] = {
    ...
    def _apply_dynamic_curve_visibility(self):
        ...
        for sc in getattr(self, 'bot_scatters', []):
            key = f"bot:{sc.name()}"
            if key in legend: sc.setVisible(legend[key])
    ...
    def _on_mouse_moved(self, pos):
        ...
        if trades_here:
            ...
            for tr in trades_here:
                ...
                if not is_b and tr.get('buyer'):
                    bot = tr['buyer']
                    color = getattr(self, 'bot_colors', {}).get(bot, DIM)
                    row_line(f'BOT BUY ({bot})', f'{pr} &times; {qty}', color)
                if not is_s and tr.get('seller'):
                    bot = tr['seller']
                    color = getattr(self, 'bot_colors', {}).get(bot, DIM)
                    row_line(f'BOT SELL ({bot})', f'{pr} &times; {qty}', color)
            lines.append('')
    ...
    def _import_data(self):
        ...
        self.cb_prod.clear(); self.cb_prod.addItems(products + ['[Linear Combo]']); self.cb_day.clear(); self.cb_day.addItems(['All'] + [str(d) for d in df['day'].unique().sort().to_list()])
        self.cb_prod.blockSignals(False); self.cb_dash_prod.clear(); self.cb_dash_prod.addItems(['Overall'] + products)
        self._apply_saved_settings()
        self._build_custom_plots(); self._build_position_plot(); self._build_logs_table()
        self._handle_product_or_day_change()
        self._apply_dynamic_curve_visibility(); self._update_dashboard()
    ...
    def _load_file(self, path):
        ...
        self.cb_prod.blockSignals(True); products = df['product'].unique().sort().to_list(); self.cb_prod.clear(); self.cb_prod.addItems(products + ['[Linear Combo]'])
        ...
        self._build_custom_plots(); self._build_position_plot(); self._build_logs_table(); self._update_sandbox_table()
        self._handle_product_or_day_change()
        self._apply_dynamic_curve_visibility(); self._update_dashboard()
    ...
    def _handle_product_or_day_change(self):
        if not self.data or not self.cb_prod.currentText(): return
        prod = self.cb_prod.currentText()
        day = self.cb_day.currentText()

        if prod == '[Linear Combo]':
            self.cb_vol.clear()
            ...
            for tr in self.data['trades']:
                if tr.get('symbol') != prod: continue
                if day != 'All' and 'day' in tr and str(tr['day']) != day: continue
                vols.add(abs(int(tr.get('quantity', 0))))
            
            self.cb_vol.blockSignals(True)
            self.cb_vol.clear()
            for v in sorted(list(vols)):
                self.cb_vol.addCheckableItem(str(v), checked=True)
            self.cb_vol.blockSignals(False)
    ...
    def _render_plot(self):
        ...
        for item in getattr(self, 'bot_scatters', []):
            try:
                self.p_m.removeItem(item)
                self.leg_m.removeItem(item)
            except: pass
        self.bot_scatters = []
        self.bot_colors = {}

        if prod == '[Linear Combo]':
            all_products = [self.cb_prod.itemText(i) for i in range(self.cb_prod.count())
                            if self.cb_prod.itemText(i) != '[Linear Combo]']
            self._combo_panel.set_assets(all_products)
            self._combo_panel.setVisible(True)
            self.curve_mid.setData([], [])
            self.curve_pnl.setData([], [])
            self.sc_buy.setData(x=[], y=[])
            self.sc_sell.setData(x=[], y=[])
            self.img_item.setVisible(False); self.img_orders.clear()
            self.current_df = None; self._plot_t = None
            coeffs = self._combo_panel.get_coefficients()
            if coeffs:
                self._plot_linear_combo(coeffs)
            elif self._combo_curve:
                self._combo_curve.setData([], [])
                self._combo_panel.reset_stats()
                if self._combo_mean_line:
                    self._combo_mean_line.setVisible(False)
            return

        self._combo_panel.setVisible(False)
        if self._combo_curve:
            self._combo_curve.setData([], [])
        if self._combo_mean_line:
            self._combo_mean_line.setVisible(False)
        self._combo_panel.reset_stats()
        ...
        selected_vols = set([int(x) for x in self.cb_vol.getCheckedItems()])
        ...
        for tr in self.data['trades']:
            ...
            qty = abs(int(tr.get('quantity', 1)))
            if qty not in selected_vols: continue
            ...
            if is_b: mb_t.append(ts); mb_p.append(pr)
            elif is_s: ms_t.append(ts); ms_p.append(pr)
            
            if not is_b and 'buyer' in tr and tr['buyer']:
                bot = tr['buyer']
                if bot not in bot_buys: bot_buys[bot] = {'t':[], 'p':[]}
                bot_buys[bot]['t'].append(ts); bot_buys[bot]['p'].append(pr)
                
            if not is_s and 'seller' in tr and tr['seller']:
                bot = tr['seller']
                if bot not in bot_sells: bot_sells[bot] = {'t':[], 'p':[]}
                bot_sells[bot]['t'].append(ts); bot_sells[bot]['p'].append(pr)
                
            if not is_b and not is_s:
                bot_raw.append((ts, pr, qty))
        ...
        for i, bot in enumerate(unique_bots):
            ...
            if bot in bot_buys and bot_buys[bot]['t']:
                sc_b = pg.ScatterPlotItem(x=bot_buys[bot]['t'], y=bot_buys[bot]['p'], symbol='t1', size=10, brush=color, name=f"{bot} BUY", pen=pg.mkPen('k', width=0.5))
                sc_b.setZValue(2)
                self.p_m.addItem(sc_b)
                self.leg_m.addItem(sc_b, f"{bot} BUY")
                self.bot_scatters.append(sc_b)
                
            if bot in bot_sells and bot_sells[bot]['t']:
                sc_s = pg.ScatterPlotItem(x=bot_sells[bot]['t'], y=bot_sells[bot]['p'], symbol='t', size=10, brush=color, name=f"{bot} SELL", pen=pg.mkPen('k', width=0.5))
                sc_s.setZValue(2)
                self.p_m.addItem(sc_s)
                self.leg_m.addItem(sc_s, f"{bot} SELL")
                self.bot_scatters.append(sc_s)
    ...
    def _plot_linear_combo(self, coeffs: dict):
        if not self.data:
            return
        day = self.cb_day.currentText()
        cont_ts = self.data.get('_continuous_ts', False)
        min_day = self.data.get('_min_day', 0)
        df_all = self.data['prices_df']

        series: list[tuple[np.ndarray, np.ndarray]] = []
        coeff_vals: list[float] = []
        for asset, coeff in coeffs.items():
            flt = df_all.filter(pl.col('product') == asset)
            try:
                if day != 'All':
            ...
            if flt.is_empty():
                continue
            t = flt['timestamp'].to_numpy().astype(float)
            if day == 'All' and 'day' in flt.columns and not cont_ts:
                t = t + (flt['day'].to_numpy() - min_day) * 1_000_000
            mid = flt['mid_price'].to_numpy().astype(float)
            valid = np.isfinite(mid)
            if not np.any(valid):
                continue
            series.append((t[valid], mid[valid]))
            coeff_vals.append(coeff)

        if not series:
            return

        all_times = np.unique(np.concatenate([s[0] for s in series]))
        combo = np.zeros(len(all_times), dtype=float)
        for (t_arr, mid_arr), coeff in zip(series, coeff_vals):
            interp = np.interp(all_times, t_arr, mid_arr,
                               left=mid_arr[0], right=mid_arr[-1])
            combo += coeff * interp

        if self._combo_curve is None:
            self._combo_curve = self.p_m.plot(
                pen=pg.mkPen(ACCENT_GOLD, width=2),
                name="Combo Mid",
                clipToView=True
            )
            self.leg_m.addItem(self._combo_curve, "Combo Mid")

        self._combo_curve.setData(all_times, combo)
        self._plot_t = all_times
        self.p_m.autoRange()

        mean_val = float(np.mean(combo))
        std_val  = float(np.std(combo))
        self._combo_panel.update_stats(mean_val, std_val)

        if self._combo_mean_line is None:
            self._combo_mean_line = pg.InfiniteLine(
                angle=0, movable=False,
                pen=pg.mkPen(ACCENT_GOLD, width=1, style=Qt.PenStyle.DashLine),
                label=f"μ={mean_val:.4f}",
                labelOpts={'position': 0.97, 'color': ACCENT_GOLD,
                           'fill': (13, 15, 20, 180), 'movable': False}
            )
            self.p_m.addItem(self._combo_mean_line)
        self._combo_mean_line.setValue(mean_val)
        self._combo_mean_line.label.setFormat(f"μ = {mean_val:.4f}")
        self._combo_mean_line.setVisible(True)

        terms = " + ".join(f"{c:+g}×{a}" for a, c in coeffs.items())
        self.p_m.setTitle(f"<span style='color:{ACCENT_GOLD}'>Combo: {terms}</span>",
                          size='8pt')
