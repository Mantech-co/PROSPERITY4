        ...
        for product in state.order_depths:
            ...
            if product == Product.EMERALDS:
                ...
                if product in self.params:
                    ...
                    # WallMid Logic
                    # Find price layer with the highest volume for bids and highest absolute volume for asks
                    bid_vol_wall = max(bid_orders, key=lambda x: x[1])[0]
                    ask_vol_wall = min(ask_orders, key=lambda x: abs(x[1]))[0]
                    
                    wall_mid = (bid_vol_wall + ask_vol_wall) / 2.0
                    fair_value = wall_mid
